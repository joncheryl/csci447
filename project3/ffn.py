...
class ffNetwork:
    ...
    def __init__(self, sizes, learningRate, momentum = 0, act = 0, output="real"):
        '''
    ...
    def getBiases(self):
        ...
        for layer in self.nodes[1:]:
            for node in layer:            
                biases.append(node.bias[0])
        return(biases)
        
    def getOutput(self):
        outVals = np.empty(self.sizes[-1])
        classReturn = np.empty(self.sizes[-1])
        classReturn.fill(0)
        for i in range(len(self.nodes[-1])):
           outVals[i] = self.nodes[-1][i].output
        classReturn[outVals.argmax()] = 1
        if self.outputType == "binary":           
            return(classReturn)
        else:
            return(outVals)
    ...
    def setWeights(self, weightVector):
        for j in range(len(self.edges)):
            self.edges[j].weight[0] = weightVector[j]
        return
        
    def setBiases(self, biasVector):
        i = 0
        for layer in self.nodes[1:]:
            for node in layer:            
                node.bias[0] = biasVector[i]
                i += 1        
        return
